...
class ConcreteStage(BaseStage):
    """Concrete implementation of BaseStage for testing"""
    ...
    def set_behavior(self, should_succeed=True, return_data=None, exception=None, stage_name=None):
        """Helper method to configure test behavior"""
...
class TestBaseStage:
    ...
    def test_execute_with_none_context(self, concrete_stage):
        """Test execution with None context (should not happen in practice)"""
        result = concrete_stage.execute(None)
        
        assert result.success is True

    def test_execute_with_complex_data(self, concrete_stage):
        """Test execution with complex return data"""
        context = {"config": {"test": "data"}}
        complex_data = {
            "scraped_items": [
    ...
    def test_logger_usage(self, mock_logger):
        """Test that logger is properly stored and accessible"""
        stage = ConcreteStage(mock_logger)
    ...
    def test_multiple_executions(self, concrete_stage):
        """Test multiple executions of the same stage"""
        ...
        assert result1.success == result2.success  # Same success status

    def test_stage_behavior_modification(self, concrete_stage):
        """Test that stage behavior can be modified between executions"""
        context = {"test": "data"}
    ...
    def test_stage_inheritance_structure(self, concrete_stage):
        """Test that concrete stage properly inherits from BaseStage"""
