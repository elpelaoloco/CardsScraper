...
class BaseScraperTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if not scraper_config:
            raise ValueError(
                f"No se encontró configuración para el tipo '{cls.SCRAPER_TYPE}'")
        ...
        if not cls.category:
            raise ValueError(
                f"No se encontró categoría '{cls.CATEGORY_KEY}' para el scraper '{cls.SCRAPER_TYPE}'")

    def test_url_is_valid(self):
        self.assertTrue(self.category['url'].startswith(
            'http'), f"⚠️ URL inválida: {self.category['url']}")
        ...
        for key in required_keys:
            with self.subTest(key=key):
                self.assertIn(key, selectors,
                              f"❌ Falta el selector requerido '{key}' en la categoría")
                self.assertTrue(selectors[key], f"❌ Selector '{key}' está vacío")

    def test_product_selector_priority(self):
        # Este test evalúa si el 'urls_selector' es parte del 'product_selector' o
        # está bien estructurado
        product_selector = self.category['selectors'].get('product_selector', '')
        urls_selector = self.category['selectors'].get('urls_selector', '')

        self.assertTrue(product_selector or urls_selector,
                        "❌ No hay selectores definidos para ubicar productos.")
