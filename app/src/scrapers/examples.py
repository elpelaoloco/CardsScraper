...
class AmazonProductScraper(BaseScraper):
    ...
    def extract_product_urls(self, category: Category) -> List[Tuple[str, str]]:
        ...
        product_selector = category.selectors.get(
            'product_selector', '.s-result-item[data-component-type="s-search-result"]')
        ...
        if not self.wait_for_element(product_selector):
            self.logger.error(
                f"Couldn't find product elements for category {category.name}")
            return []
        ...
        for product in products:
            try:
                ...
                ) if title_elements else "Unknown Product"
    ...
    def process_product(self, product_url: str, category: Category) -> Dict[str, Any]:
        ...
        try:
            ...
            self.take_screenshot(
                f"amazon_product_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.png")

            # Extract product details
            price_selector = category.selectors.get(
                'price_selector', '#priceblock_ourprice, .a-price .a-offscreen')
            rating_selector = category.selectors.get(
                'rating_selector', '.a-star-rating-wrapper, .a-icon-star')
            availability_selector = category.selectors.get(
            ...
            try:
                ...
                if price_elements:
                    price_text = price_elements[0].text.strip() if hasattr(
                        price_elements[0], 'text') else price_elements[0].get_attribute('textContent').strip()
            ...
            try:
                availability_elements = self.driver.find_elements(
                    By.CSS_SELECTOR, availability_selector)
