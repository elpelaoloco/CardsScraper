...
class BaseTCGMatcher(ABC):
    ...
    def find_matches_in_group(self, query_card, candidate_group, threshold=0.85):
        ...
        for candidate in candidate_group:
            ...
            scores['full_match'] = jaro_winkler_similarity(
                query_processed, candidate_processed)
            ...
            scores['token_match'] = sum(token_scores) / \
                len(token_scores) if token_scores else 0
            ...
            scores['sorted_match'] = jaro_winkler_similarity(
                query_sorted, candidate_sorted)
...
class YugiohCardMatcher(BaseTCGMatcher):
    ...
    def preprocess_card_name(self, name):
        ...
        # Remover ediciones
        name = re.sub(r'\b(1st|unlimited|limited)\s*edition\b',
...
class UniversalTCGMatcher:
    ...
    def detect_tcg_type(self, card_names):
        ...
        magic_indicators = ['magic', 'mtg',
                            'planeswalker', 'instant', 'sorcery', 'mana']

        pokemon_score = sum(
            1 for indicator in pokemon_indicators if indicator in sample_text)
        yugioh_score = sum(
            1 for indicator in yugioh_indicators if indicator in sample_text)
        magic_score = sum(
            1 for indicator in magic_indicators if indicator in sample_text)
