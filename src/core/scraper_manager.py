...
class ScraperManager:
        ...
        try:
            ...
            for name, scraper_config in config.get('scrapers', {}).items():
                try:
                    ...
                    self.scrapers[name] = ScraperFactory.create_scraper(
                        name, scraper_config)
                    self.logger.info(f"Created scraper: {name}")
                except Exception as e:
                    self.logger.error(
                        f"Failed to create scraper {name}: {e}", exc_info=True)
        except Exception as e:
    ...
    def save_results_per_category(self, scraper_name: str,
        ...
        for category_name, entries in results.items():
            ...
            file_path = os.path.join(
                folder_path, f'results_{scraper_name}_{category_name}.json')
            with open(file_path, "w", encoding="utf-8") as f:
                json.dump(entries, f, indent=2, ensure_ascii=False)

            self.logger.info(
                f"Saved results for {scraper_name} {category_name} at {file_path}")
